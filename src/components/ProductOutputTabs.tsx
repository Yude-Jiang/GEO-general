  ...
  return (
    <div className="rounded-2xl border border-slate-100 overflow-hidden mb-6">
        ...
        {signals.map(({ key, label, higherIsBetter }) => {
          ...
          return (
            <div key={key} className="grid grid-cols-[1fr_auto_auto_auto] items-center px-4 py-2.5 gap-3">
              ...
              <span className="text-[11px] font-black tabular-nums w-8 text-right text-blue-900">{a}</span>
      ...
      <div className="bg-slate-50 px-4 py-2 border-t border-slate-100 grid grid-cols-[1fr_auto_auto_auto] gap-3">
        ...
        <span className="text-[9px] font-black text-blue-900 tabular-nums w-8 text-right">{after.wordCount}</span>
        <div className="w-16" />
      </div>
    </div>
  );
...
const buildFooter = () => {
  ...
  return `\n\n---\n\n*Date: ${date} © 2026 GEO Strategic Hub*`;
};
    ...
    <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col h-[850px] animate-fade-in relative">
      ...
      <div className="bg-slate-50 px-6 py-4 border-b border-slate-100 flex items-center justify-between">
        <div className="flex bg-slate-200/50 p-1 rounded-xl">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'preview' ? 'bg-white text-blue-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            <Eye className="w-4 h-4" /> {p.tabPreview}
          </button>
          <button
            onClick={() => setActiveTab('analysis')}
            className={`flex items-center gap-2 px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'analysis' ? 'bg-white text-blue-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            <BarChart3 className="w-4 h-4" /> {p.tabAnalysis}
          </button>
          <button
            onClick={() => setActiveTab('schema')}
            className={`flex items-center gap-2 px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'schema' ? 'bg-white text-blue-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
        ...
        <div className="flex items-center gap-2">
          {activeTab === 'preview' && (
            <>
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-900 to-blue-950 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all disabled:opacity-30 disabled:hover:scale-100"
              >
                {isHumanizing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5 text-blue-400" />}
                {p.humanizeBtn}
              </button>

              <div className={`relative flex items-center gap-2 px-4 py-2 bg-slate-100 text-blue-900 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${(isTranslating || !content) ? 'opacity-30 cursor-not-allowed' : 'hover:bg-slate-200 cursor-pointer'}`}>
          ...
          <button
            onClick={handleCopy}
            className="p-2 bg-slate-100 text-slate-500 rounded-xl hover:bg-blue-500/10 hover:text-blue-500 transition-all relative"
            title={p.copy}
          >
      ...
      <div className="flex-1 overflow-y-auto p-10 bg-slate-50/30 custom-scrollbar">
          ...
          <article className="prose prose-slate max-w-none prose-lg prose-p:mb-8 prose-p:leading-[1.85] prose-headings:font-black prose-headings:uppercase prose-headings:tracking-tight prose-headings:mt-12 prose-headings:mb-6 prose-h2:text-2xl prose-h3:text-xl prose-a:text-blue-500 prose-code:bg-slate-100 prose-pre:bg-blue-900 prose-pre:text-white prose-pre:rounded-2xl prose-pre:shadow-lg prose-li:mb-4 prose-ul:my-6 prose-ol:my-6 prose-strong:text-blue-900 prose-blockquote:border-blue-500 prose-blockquote:bg-slate-50 prose-blockquote:rounded-xl prose-hr:my-12">
            ...
            {content && (
              <div className="not-prose mt-12 pt-5 border-t border-slate-200 text-[10px] text-slate-400 font-mono tracking-tight">
                Date: {new Date().toISOString().slice(0, 10)} © 2026 GEO Strategic Hub
              </div>
            )}
          </article>
        )}
        ...
        {activeTab === 'analysis' && (
          <div className="space-y-6">
            ...
            <article className="prose prose-slate max-w-none prose-lg prose-p:mb-8 prose-p:leading-[1.85] prose-headings:font-black prose-headings:text-blue-900 prose-headings:uppercase prose-headings:tracking-tight prose-headings:mt-10 prose-headings:mb-6 prose-h2:text-2xl prose-h3:text-xl prose-li:mb-4 prose-ul:my-6 prose-ol:my-6 prose-strong:text-blue-900 prose-blockquote:border-blue-500 prose-blockquote:bg-blue-50 prose-blockquote:rounded-xl prose-hr:my-12">
      ...
      <div className="bg-white px-6 py-3 border-t border-slate-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{p.footerNote || 'Content grounded to source materials • AI citation optimized'}</span>
        </div>
        <button
          onClick={handleExport}
          disabled={!content}
          className="flex items-center gap-1 text-[9px] font-black text-blue-500 uppercase tracking-widest hover:underline disabled:opacity-30 disabled:no-underline"
        >
