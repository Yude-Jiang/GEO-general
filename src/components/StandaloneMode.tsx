...
import { getGeoMethods, getRecommendedCombos } from '../services/geoMethods';
...
const StandaloneMode: React.FC<{ t: TranslationKeys }> = ({ t }) => {
  ...
  const [selectedMethods, setSelectedMethods] = useState<GeoMethodId[]>(() =>
    Object.values(getRecommendedCombos())[0]?.ids ?? []
  );
  ...
  const handleOptimize = async () => {
    ...
    const techPattern = getIndustryConfig().techTermsPattern;
    const geoSignalsBefore = computeGeoSignals(fullContent, techPattern);
    ...
    try {
      ...
      if (content.trim()) {
        const geoSignalsAfter = computeGeoSignals(content, getIndustryConfig().techTermsPattern);
  ...
  const handleGenerateReport = async () => {
    ...
    const selectedMethodLabels = selectedMethods
      .slice(0, 3)
      .map(id => getGeoMethods().find(m => m.id === id)?.label || id);
    try {
  ...
  const tabClass = (tab: InputTab) =>
    `px-4 py-2 text-[10px] font-black uppercase tracking-widest rounded-t-lg transition-all border-b-2 ${
      inputTab === tab
        ? 'text-blue-900 border-blue-500 bg-white'
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
            <div className="bg-blue-900 px-6 py-4 flex items-center justify-between">
              <h3 className="text-white font-black uppercase tracking-widest text-xs flex items-center gap-2">
                <FileText className="w-4 h-4 text-blue-400" /> {t.standalone.contentSourceTitle}
              </h3>
            ...
            <div className="p-5 space-y-4">
              ...
              {inputTab === 'text' && (
                <div className="space-y-3">
                  <textarea
                    ...
                    className="w-full h-40 bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-medium text-blue-900 placeholder-slate-400 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all resize-none"
                  />
                  <button
                    onClick={handleAddPastedText}
                    disabled={!pastedText.trim()}
                    className="w-full py-2.5 bg-blue-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-950 disabled:opacity-30 transition-all"
                  >
                ...
                <div className="space-y-3">
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-950 disabled:opacity-30 transition-all shadow-md"
                    >
                      {isUrlLoading
              ...
              {/* File Upload Tab */}
              {inputTab === 'file' && (
                <label className="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl py-8 hover:bg-slate-50 hover:border-blue-500/30 cursor-pointer transition-all group">
                  <Upload className="w-6 h-6 text-slate-300 group-hover:text-blue-500 mb-2 transition-colors" />
                  <span className="text-[10px] font-black uppercase text-slate-400 group-hover:text-blue-900">
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-5 space-y-3">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
              <Target className="w-3 h-3 text-blue-500" /> {t.standalone.directiveLabel}
            </label>
            <textarea
              value={directive}
              onChange={e => setDirective(e.target.value)}
              placeholder={t.standalone.directivePlaceholder}
              className="w-full h-20 bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-medium text-blue-900 placeholder-slate-400 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all resize-none"
            />
          </div>
            ...
            <div className="space-y-3">
              ...
              >
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5 cursor-pointer">
                  <Zap className="w-3 h-3 text-blue-400" /> {t.standalone.geoMethodsLabel}
                  ...
                  {selectedMethods.length > 0 && (
                    <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">
                      {selectedMethods.length} selected · +{(() => {
                        const lifts = selectedMethods.slice(0, 3).map(id => parseInt(getGeoMethods().find(m => m.id === id)?.liftEstimate || '10'));
              ...
              {methodsExpanded && (
                <>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {Object.entries(getRecommendedCombos()).map(([key, combo]) => (
                      <button
                        key={key}
                        ...
                        className={`text-[9px] font-black uppercase tracking-wider px-2.5 py-1 rounded-lg border transition-all ${
                          JSON.stringify(selectedMethods) === JSON.stringify(combo.ids)
                            ...
                            : 'bg-slate-50 text-slate-500 border-slate-200 hover:border-blue-500 hover:text-blue-500'
                        }`}
                      >
                        {combo.label}
                      </button>
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl border text-left transition-all ${
                            isSelected
                              ? 'bg-blue-500/8 border-blue-500 text-blue-900'
                              : isDisabled
            ...
            <button
              ...
              className={`w-full font-black text-sm uppercase py-5 rounded-2xl transition-all flex items-center justify-center gap-3 shadow-lg group ${
                output.retryCountdown
                  ...
                  : 'bg-blue-900 text-white hover:bg-blue-950 disabled:opacity-40 disabled:cursor-not-allowed'
              }`}
            >
              {output.retryCountdown ? (
                ...
                <><Zap className="w-5 h-5 text-blue-400 group-hover:scale-110 transition-transform" /> {t.standalone.optimizeBtn}</>
              )}
            </button>
          </div>
        </div>
          ...
          {output.content && (
            <div className="flex justify-end mb-3">
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-500 to-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:shadow-md transition-all disabled:opacity-50"
              >
                {isGeneratingReport
