...
const StepProduction: React.FC<{ t: TranslationKeys }> = ({ t }) => {
  ...
  const handleGenerate = async (targetIdx: number) => {
    ...
    try {
      ...
      const geoSignalsBefore = computeGeoSignals(rawSourceText, getIndustryConfig().techTermsPattern);
      ...
      if (content.trim()) {
        updateBundle(targetIdx, { geoSignalsAfter: computeGeoSignals(content, getIndustryConfig().techTermsPattern) });
      }
    ...
    <div className="space-y-6 animate-fade-in pb-20">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-xl border border-slate-100 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <button
            onClick={() => setStep(2)}
            className="p-3 bg-slate-50 text-slate-400 hover:text-blue-900 rounded-2xl transition-all group"
          >
            <ArrowLeft className="w-5 h-5 group-hover:-translate-x-1" />
          </button>
          <div>
            <h2 className="text-xl font-black uppercase tracking-tight text-blue-900 flex items-center gap-2">
              <Zap className="w-5 h-5 text-blue-400" /> {t.production.title}
            </h2>
              ...
              {bundleOutputs.map((o, i) => (
                <button
                  ...
                  className={`w-2.5 h-2.5 rounded-full transition-all ${
                    o.isGenerating ? 'bg-blue-500 animate-pulse scale-125' :
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs font-bold text-blue-900 truncate leading-snug mt-0.5">
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-6 space-y-6">
            ...
            <div className="space-y-3">
              ...
              >
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5 cursor-pointer">
                  <Zap className="w-3 h-3 text-blue-400" /> {t.production.geoMethodsLabel}
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                <Edit3 className="w-3 h-3 text-blue-500" /> {t.production.customPromptLabel}
              </label>
              <textarea
                value={customPrompt}
                onChange={(e) => setCustomPrompt(e.target.value)}
                placeholder={t.production.customPromptPlaceholder}
                className="w-full h-24 bg-slate-50 border border-slate-200 rounded-2xl p-4 text-sm font-medium text-blue-900 placeholder-slate-400 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all resize-none shadow-inner"
              />
            </div>
              ...
              className={`w-full font-black text-sm uppercase py-5 rounded-2xl transition-all flex items-center justify-center gap-3 shadow-lg group ${
                activeOutput.retryCountdown
                  ...
                  : activeOutput.isDeepSearching
                    ...
                    : 'bg-blue-900 text-white hover:bg-blue-950 disabled:opacity-40'
              }`}
            >
              {activeOutput.retryCountdown ? (
                <><Loader2 className="w-5 h-5 animate-spin" /> {t.production.rateLimitPrefix} {activeOutput.retryCountdown}s</>
              ) : activeOutput.isDeepSearching ? (
                <><SearchIcon className="w-5 h-5 animate-pulse text-blue-400" /> {t.production.deepGrounding}</>
              ) : activeOutput.isGenerating ? (
                <><Loader2 className="w-5 h-5 animate-spin" /> {t.production.generatingBtn}</>
              ) : (
                <><Zap className="w-5 h-5 text-blue-400" /> {t.production.generateBtn}</>
              )}
            </button>
            ...
            {!isFreeform && bundles.length > 1 && (
              <button
                ...
                className="w-full font-black text-xs uppercase py-3 rounded-xl transition-all flex items-center justify-center gap-2 border-2 border-slate-200 text-slate-500 hover:border-blue-500 hover:text-blue-500 disabled:opacity-30"
              >
