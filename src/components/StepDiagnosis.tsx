    ...
    <div className="space-y-6 animate-fade-in relative">
      <div className="bg-gradient-to-br from-blue-900 to-blue-950 rounded-2xl p-8 text-white shadow-2xl relative overflow-hidden">
        ...
        <p className="text-gray-400 text-sm mb-6">
          {t.diagnosis.subtitle} <span className="text-blue-400 font-bold">{targetEcosystem.toUpperCase()}</span> {t.diagnosis.subtitleSuffix}
        </p>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <textarea
              className="w-full h-48 bg-blue-950/60 border border-blue-500/20 rounded-xl p-4 text-white placeholder-gray-400/50 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all resize-none shadow-inner"
              placeholder={getIndustryConfig().exampleSeeds[targetEcosystem] || getIndustryConfig().exampleSeeds.global}
              value={inputText}
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                className="w-full bg-blue-950/60 border border-blue-500/20 rounded-xl p-3 text-white placeholder-gray-400/50 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <button
              ...
              className="w-full bg-blue-500 text-white font-black text-sm uppercase tracking-wider py-4 rounded-xl hover:bg-blue-600 transition-all flex items-center justify-center gap-2"
            >
        ...
        <div className="space-y-8 pb-32">
          ...
          <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            <div className="bg-gradient-to-r from-blue-900 to-blue-950 p-6 text-white">
              ...
              <p className="text-[11px] text-blue-500 font-medium opacity-90 leading-relaxed max-w-2xl">
          ...
          {(() => {
            ...
            return (
              <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
                {/* Header */}
                <div className="bg-gradient-to-r from-blue-950 to-blue-900 p-5 flex items-center justify-between">
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {mmv.snapshots.map(snap => (
                          <div key={snap.modelId} className={`rounded-xl border p-4 ${snap.error ? 'border-slate-200 bg-slate-50' : 'border-blue-500/20 bg-blue-50'}`}>
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-xs font-black text-blue-900 uppercase">{snap.modelName}</span>
                              {snap.error
                            ...
                            {snap.error ? (
                              ...
                              <>
                                <div className="text-xs text-slate-600 leading-relaxed mb-3 max-h-96 overflow-y-auto whitespace-pre-wrap break-words">
                                  {snap.rawResponse}
                                </div>
                                {snap.keyEntities.length > 0 && (
                                  <div className="flex flex-wrap gap-1">
                                    {snap.keyEntities.slice(0, 8).map(e => (
                                      <span key={e} className="text-[10px] bg-blue-500/10 text-blue-950 border border-blue-500/20 px-2 py-0.5 rounded-full font-bold">
                                        {e}
                                      </span>
                                    ))}
                                  </div>
          ...
          {diagnosisResult.competitorAnalysis && diagnosisResult.competitorAnalysis.length > 0 && (
            <div className="space-y-4 pt-4">
              <div className="mb-6 px-2">
                <h3 className="text-blue-900 font-black uppercase tracking-widest text-sm flex items-center gap-2 mb-2">
                ...
                {diagnosisResult.competitorAnalysis.map((comp, idx) => (
                  <div key={idx} className="bg-white rounded-3xl p-6 shadow-lg border border-rose-100 hover:shadow-2xl transition-all relative overflow-hidden group">
                    ...
                    <div className="flex justify-between items-start mb-6">
                      <h4 className="text-xl font-black text-blue-900 uppercase">{comp.competitorName}</h4>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-xs text-slate-700 font-medium leading-relaxed italic bg-slate-50 p-2 rounded-lg border-l-2 border-blue-500">"{comp.aiPerception}"</p>
                      </div>
                      <div>
              ...
              {diagnosisResult.intentClusters?.map((cluster, cIdx) => (
                <div key={cIdx} className={`bg-white rounded-3xl shadow-xl border flex flex-col md:flex-row overflow-hidden hover:shadow-2xl transition-all ${
                  ...
                  <div className="w-full md:w-80 bg-slate-50 p-8 border-r border-slate-100">
                    <h4 className="text-xl font-black text-blue-900 mb-3 uppercase">{cluster.intentName}</h4>
                  ...
                  <div className="flex-1 p-0 bg-white">
                    <div className="hidden md:grid grid-cols-12 gap-4 bg-slate-50/50 p-4 border-b border-slate-100 items-center">
                       ...
                       <div className="col-span-5 text-[10px] font-black text-slate-400 uppercase tracking-widest"><Crosshair className="w-3 h-3 inline mr-1"/> Expected AI Anchor</div>
                    </div>
                    <div className="divide-y divide-slate-50">
                      {cluster.monitoringQuestions?.map((q) => (
                        <label
                          key={q.id}
                          className={`group flex flex-col md:grid md:grid-cols-12 gap-4 p-5 md:items-center cursor-pointer transition-colors ${selectedItems.has(q.id) ? 'bg-blue-500/5' : 'hover:bg-slate-50/50'}`}
                        >
                          <div className="col-span-1 flex justify-center mt-2 md:mt-0">
                            ...
                            {selectedItems.has(q.id) ? <CheckCircle2 className="w-5 h-5 text-blue-500" /> : <div className="border-2 border-slate-200 rounded-full w-5 h-5 group-hover:border-blue-500/50 transition-colors" />}
                          </div>
                          <div className="col-span-6 text-sm font-black text-blue-900 leading-snug">{q.userPrompt}</div>
          ...
          <div className="fixed bottom-10 left-1/2 -translate-x-1/2 w-[90%] max-w-4xl z-50">
            <div className="bg-slate-900 text-white rounded-2xl shadow-xl p-6 flex items-center justify-between">
              <div>
                <p className="text-sm font-black uppercase tracking-widest"><Lock className="w-4 h-4 inline mr-2 text-blue-400" />{selectedItems.size} Selected</p>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="bg-slate-800 border border-blue-500/30 text-blue-400 font-black text-xs uppercase px-6 py-4 rounded-xl hover:bg-blue-500 hover:text-white transition-all flex items-center gap-2"
                >
                ...
                <button
                  ...
                  disabled={selectedItems.size === 0}
                  className="bg-blue-500 text-white font-black text-sm uppercase px-10 py-4 rounded-xl hover:bg-blue-600 transition-all flex items-center gap-3"
                >
      ...
      {isRefiningStrategy && (
        <div className="fixed inset-0 z-[100] bg-blue-900/95 backdrop-blur-md flex flex-col items-center justify-center p-8 animate-fade-in shadow-2xl">
          ...
          <p className="text-gray-400 font-bold text-sm max-w-md text-center leading-relaxed">
            {(t.diagnosis as any).battleRoomSub.split('{count}').map((part: string, index: number, array: string[]) => (
              <React.Fragment key={index}>
                {part}
                {index < array.length - 1 && (
                  <span className="text-blue-500 font-black">{selectedMonitoringQuestions.length}</span>
                )}
              </React.Fragment>
            ))}
            <br />
