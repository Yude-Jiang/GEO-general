    ...
    <>
      {!isOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 w-14 h-14 bg-blue-900 text-white rounded-full shadow-2xl flex items-center justify-center hover:bg-blue-950 transition-all z-50 animate-bounce"
        >
        ...
        <div className="fixed bottom-6 right-6 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-slate-200 z-50 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-5 transition-all h-[500px] max-h-[80vh]">
          ...
          <div className="p-3 bg-white border-t border-slate-100 flex items-center gap-2">
            <input
              ...
              className="flex-1 bg-slate-100 border-none rounded-full px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              disabled={isLoading}
            />
            <button
              onClick={handleSend}
