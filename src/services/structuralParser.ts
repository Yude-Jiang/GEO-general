...
const QUANT_RE = /\d+\.?\d*\s*(μA|mA|A|MHz|GHz|kHz|kB|KB|MB|GB|ms|μs|ns|°C|V|mV|W|mW|dBm|bps|Mbps|%|\$|USD|RMB|元)|(?:\$|USD|RMB|元)\s*\d+\.?\d*/gi;
...
const DEFAULT_TECH_RE = /\b(Cortex-M\d+\+?|TrustZone|TEE|BLE\s*\d|Matter\s*\d|Thread|Zigbee|PSA\s*Certified|CC\s*EAL\d|SESIP\d|AEC-Q\d+|JEDEC|secure\s*boot|Root\s*of\s*Trust|datasheet|application\s*note|AN\d{4}|STM32\w+|nRF\d+|ESP\d+|RP\d+)\b/gi;
const HIGH_WEIGHT_TYPES: ChunkType[] = ['spec_table', 'dense_fact', 'code_example', 'comparison'];

export function computeGeoSignals(text: string, techTermsPattern?: string): GeoSignals {
  if (!text || !text.trim()) {
    return { quantifiedClaims: 0, hedgeWords: 0, techTerms: 0, citableChunks: 0, blufCompliance: false, wordCount: 0 };
  }
  const techRe = techTermsPattern !== undefined && techTermsPattern !== null
    ? (techTermsPattern ? new RegExp(techTermsPattern, 'gi') : /(?!)/)
    : DEFAULT_TECH_RE;
  ...
  return {
    ...
    techTerms:        (text.match(techRe)  || []).length,
